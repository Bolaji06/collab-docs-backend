...
export class DocumentService {
     ...
     */
    async getDocumentsForUser(userId, options = {}) {
        const where = {
            deletedAt: null,
        };
     ...
     */
    async getDeletedDocumentsForUser(userId) {
        return prisma.document.findMany({
            where: {
                ownerId: userId,
                NOT: {
     ...
     */
    async getDocumentById(documentId, userId, includeDeleted = false) {
     ...
     */
    async shareDocument(documentId, ownerId, email, role) {
        const document = await prisma.document.findUnique({
            where: { id: documentId },
        });
        if (!document) {
     ...
     */
    async removePermission(documentId, ownerId, userIdToRemove) {
        const document = await prisma.document.findUnique({
            where: { id: documentId },
        });
        if (!document) {
    ...
    async updateAccess(documentId, userId, data) {
        const document = await prisma.document.findUnique({
            where: { id: documentId },
        });
        if (!document) {
